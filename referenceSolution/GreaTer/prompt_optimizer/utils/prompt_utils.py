...
class PromptManager:
    def __init__(self, tokenizer, conv_template, goal, target, opt_prompt, opt_place = 'suffix'):
        """
                Initializes the PromptManager with the provided parameters.

                Parameters
                ----------
                goal : str
                    usually the input to the optimized prompt

                tokenizer : Transformer Tokenizer
                    The tokenizer used to convert text into tokens

                conv_template : Template
                    The conversation template used for the attack, changes with target LLM


                target : str
                    The target output of the total prompt and input

                opt_prompt : str, prefix/suffix prompt to be optimized
                    The added prompt that we can optimize for best performance

                opt_place : str, 'prefix' or 'suffix', by default suffix
                    The place of incorporating the opt_prompt, can be either at front or at back.
                    TODO: possibly add infix by also adding/subtracting prompt tokens inside
        ...
        self.opt_place = opt_place

    def get_prompt(self, opt_prompt, position): # front, or back
        """
        get the processed prompt ready for putting into LLM in correct format

        Parameters
        ----------
        :param opt_prompt: prompt to be optimized
        :param position: where to put the prompt, prefix or suffix.

        """
        if opt_prompt is not None:
            self.opt_prompt = opt_prompt
        if position == "prefix":
            self.conv_template.append_message(self.conv_template.roles[0], f"{self.opt_prompt} {self.goal}")
        elif position == "suffix":
            self.conv_template.append_message(self.conv_template.roles[0], f"{self.goal} {self.opt_prompt}")
        ...
        if self.conv_template.name == 'llama-2':
            ...
            if position == "prefix":
                ...
                self.conv_template.update_last_message(f"{self.opt_prompt}")
                toks = self.tokenizer(self.conv_template.get_prompt()).input_ids
                self._control_slice = slice(self._user_role_slice.stop, max(self._user_role_slice.stop, len(toks)))

                separator = ' ' if self.opt_prompt else ''
                self.conv_template.update_last_message(f"{self.opt_prompt}{separator}{self.goal}")
                toks = self.tokenizer(self.conv_template.get_prompt()).input_ids
                ...
                self.conv_template.update_last_message(f"{self.goal}{separator}{self.opt_prompt}")
                toks = self.tokenizer(self.conv_template.get_prompt()).input_ids
                self._control_slice = slice(self._goal_slice.stop, len(toks))

            # TODO plan about incorporating infix editing in the strings
            ...
            if python_tokenizer:
                ...
                self._control_slice = slice(
                    encoding.char_to_token(prompt.find(self.opt_prompt)),
                    encoding.char_to_token(prompt.find(self.opt_prompt) + len(self.opt_prompt))
                )
    ...
    def get_input_ids(self, opt_prompt):
        prompt = self.get_prompt(opt_prompt=opt_prompt)
    ...
    def logits(self, model, test_controls=None, return_ids=False):
        ...
        if isinstance(test_controls, torch.Tensor):
            ...
            # why not just use <unk> in pad_tok
            pad_tok = 0
...
class PromptsManager(object):
    ...
    def __init__(self, goals, targets, tokenizer, conv_template, control_init = "!!!!!",
                 managers=None, opt_place="prefix", *args, **kwargs):
...
class MultiPromptManager(object):
    ...
    def run(self,
        ...
        for i in range(n_steps):
            ...
            if loss < loss_limit:
                print('Current Loss reached threshold. Selecting next example to optimize: ' + str(loss))
                break

        return self.control_str, loss, steps


class ProgressiveMultiPromptManager(object):
    def __init__(self,
                 goals,
                 targets,
                 workers,
        ...
        self.logfile = logfile
        self.managers = managers
        self.mpa_kwargs = ProgressiveMultiPromptManager.filter_mpa_kwargs(**kwargs)
        self.opt_place = opt_place
    ...
    def run(self,
        ...
        loss_limit : float, optional
            The limit of loss after which we stop optimizing and move on to the next one
...
class ModelWorker():
    def __init__(self, model_path, model_kwargs, tokenizer, conv_template, device_map):
        if type(model_path) == type("sample string"):
            self.model = AutoModelForCausalLM.from_pretrained(model_path, torch_dtype=torch.float16,
                                                              trust_remote_code=True, **model_kwargs, device_map=device_map)
        else:
            self.model = model_path # to use a shared model if we want
