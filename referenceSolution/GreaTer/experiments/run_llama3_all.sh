...
#selected_tasks=("formal_fallacies" "disambiguation_qa" "tracking_shuffled_objects_five_objects" "date_understanding" "disambiguation_qa" "formal_fallacies" "geometric_shapes" "salient_translation_error_detection" "penguins_in_a_table" "causal_judgement" "logical_deduction_five_objects" "movie_recommendation" "navigate" "web_of_lies" "sports_understanding" "reasoning_about_colored_objects" "hyperbaton" "ruin_names" "snarks" "temporal_sequences" "boolean_expressions")  # Add the tasks you want to run

#selected_tasks=("tracking_shuffled_objects_five_objects" "date_understanding" "disambiguation_qa" "formal_fallacies" "geometric_shapes" "salient_translation_error_detection" "penguins_in_a_table" "causal_judgement" "logical_deduction_five_objects" "movie_recommendation" "navigate" "web_of_lies" "sports_understanding" "reasoning_about_colored_objects" "hyperbaton" "ruin_names" "snarks" "temporal_sequences" "boolean_expressions")  # Add the tasks you want to run

#selected_tasks=("formal_fallacies" "causal_judgement" "navigate" "web_of_lies" "sports_understanding" "boolean_expressions")

#selected_tasks=("salient_translation_error_detection" "ruin_names" "date_understanding" "movie_recommendation" "reasoning_about_colored_objects")

selected_tasks=("object_counting" "tracking_shuffled_objects_five_objects" "hyperbaton" "causal_judgement")
...
start_task() {
    ...
    CUDA_VISIBLE_DEVICES=$gpu_string python main.py \
        --config="./configs/transfer_llama32_1b.py" \
        --config.train_data="../data/BBH/${task_name}.json" \
        --config.test_data="../data/BBH/${task_name}.json" \
        --config.result_prefix="results/transfer_llama3_${task_name}_fffff.json" \
        ...
        > "llama32_1b-logs-2/${task_name}_fffff.log" 2>&1 &
...
for task_name in "${selected_tasks[@]}"; do
    while true; do
        ...
        available_gpus=()
        for i in $(seq 1 $((TOTAL_GPUS - 1))); do
            # Skip GPU 6
#            if [ "$i" -eq 6 ]; then
#                continue
#            fi
