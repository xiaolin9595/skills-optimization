...
class ControlPrompt(object):
    """
    A class used to generate a control prompt
    """

    def __init__(self,
                 goal, target, target_proper, tokenizer, conv_template, control_init = "!!!!!!!!!!!!!!!", control_pos = "post",
                 *args, **kwargs):
        """
        Initializes the ControlPrompt object with provided parameters
        :param goal:
            goal string -> input string prompt. This is the string that is entered as an input, before of after which we use a control prompt
        :param target:
            target string -> we try to find a suitable place in the string where the goal string has highest probability
        :param tokenizer:
            tokenizer
        :param conv_template:
            template for the target model
        :param control_init:
            control init -> the target prompt/chain of thought prompt that will result in improved performance
        :param args:
        """
        self.goal = goal
        self.target = target
        self.target_proper = target_proper
        ...
        # for substr in self.target:
        #     self.test_new_toks = max(self.test_new_toks, len(self.tokenizer(substr).input_ids))

        self._update_ids()

    # def log_perplexity(logits, prompts, prefix_len=None, ret_all=False):
    #     # shift_prompts = prompts[1:]
    #     shift_prompts = prompts[:, 0:]
    #     shift_logits = logits[:, :shift_prompts.shape[1], :]
    #     log_probs = F.log_softmax(shift_logits, dim=2)
    #     # stacked_perplexities = torch.stack([log_probs[torch.arange(shift_prompts.shape[0]), shift_prompts].mean()])
    #     stacked_perplexities = torch.stack(
    #         [log_probs[i, torch.arange(shift_prompts.shape[1]), shift_prompts[i]].mean() for i in
    ...
    def _update_ids(self):
        ...
        # self.conv_template.system = "<s>[INST] <<SYS>> <</SYS>>"
        # start_delim = "<<SYS>>"
        # end_delim = "<</SYS>>"
        # replacement = start_delim + end_delim
    ...
    def test(self, model, gen_config=None):
        ...
        matched = self.target_proper in gen_str # basically we are also looking for em
        em = self.target in gen_str
        return matched, int(em)
...
class PromptManager(object):
    ...
    def __init__(self,
        ...
        *args, **kwargs
        ...
        targets_proper : list of strs
            The list of proper targets of the attack - matchable
        ...
        control_pos: str, post or pre
            A string used to control the position of the control string, after or before the input prompt.
        ...
        self._prompts = [
            ...
            for goal, target, target_proper in zip(goals, targets, targets_proper)
        ]
...
class MultiPromptManager(object):
    ...
    def __init__(self,
        ...
        """
                ...
                control_init : str, optional
                    A string used to control the attack (default is "! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! !")
                control_pos : str, optional
                    Where the control string will go, post or pre
        ...
        self.targets_proper = targets_proper
