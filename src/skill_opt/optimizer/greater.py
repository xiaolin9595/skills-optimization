...
class GreaterOptimizer(SkillOptimizer):
    ...
    def optimize(self, skill: Skill, config: OptimizeConfig) -> OptimizedSkill:
        ...
        init_ids = self.tokenizer(
            current_control_text, add_special_tokens=False
        ).input_ids

        if len(init_ids) != config.start_len:
            logger.warning(
                f"Initial control length ({len(init_ids)}) does not match start_len ({config.start_len}). Resizing..."
            )
            if len(init_ids) > config.start_len:
                # Truncate
                init_ids = init_ids[: config.start_len]
                current_control_text = self.tokenizer.decode(
                    init_ids, skip_special_tokens=True
                )
        ...
        for length_iter in range(config.start_len, config.end_len + 1):
            ...
            if length_iter > config.start_len:
                ...
                if len(check_ids) != length_iter:
                    ...
                    # Or just warn
                    logger.warning(
                        f"Control length mismatch after extension. Expected {length_iter}, got {len(check_ids)}."
                    )
            ...
            for epoch in range(config.iterations):
                ...
                logger.info(
                    f"Len {length_iter} | Epoch {epoch + 1}/{config.iterations}"
                )
                ...
                ref_p = prompters[0]
                gen_input_list = [
                    p.input_ids[: p._assistant_role_slice.stop] for p in prompters
                ]
                ...
                for i in range(control_len_tokens):
                    ...
                    batch_pos_indices = [
                        (p._control_slice.start + i) for p in prompters
                    ]
                    ...
                    accumulated_grads = torch.zeros(
                        1, len(candidates), device=self.device
                    )
                    ...
                    for b in range(grad_samples):
                        ...
                        # Gradient is already normalized and clipped in compute_gradient
                        accumulated_grads += grad

                    # Stage 4: Selection & Update with Cache
                    # Guard against case where fewer candidates than top_mu
                    actual_top_mu = min(config.top_mu, len(candidates))
                    if actual_top_mu == 0:
                        continue
                    vals, sorted_indices = torch.topk(
                        -accumulated_grads.squeeze(), actual_top_mu
                    )
                    ...
                    for token_id in eval_tokens:
                        ...
                        if cache_key in loss_cache:
                            ...
                            with torch.no_grad():
                                ...
                                outputs = self.model(
                                    val_prompter.input_ids.unsqueeze(0)
                                )
                    ...
                    if best_cand_token is not None and best_cand_token != orig_token:
                        logger.info(
                            f"Position {i}: Updated token {self.tokenizer.decode([orig_token])} -> {self.tokenizer.decode([best_cand_token])} | Loss: {step_best_loss:.4f} (was {current_best_loss_for_len:.4f})"
                        )
                    else:
                        logger.debug(
                            f"Position {i}: No improvement (Loss: {step_best_loss:.4f})"
                        )

                    # Update all prompters
                    current_ids = (
                        prompters[0].input_ids[prompters[0]._control_slice].clone()
                    )
                    if i < len(current_ids) and best_cand_token is not None:
                    ...
                    if step_best_loss < current_best_loss_for_len:
                        logger.info(
                            f"New best loss for len {length_iter} at epoch {epoch + 1}: {step_best_loss:.4f}"
                        )
    ...
    def evaluate_skill(
        ...
        logger.info(
            f"Evaluating skill '{skill.name}' on {dataset_path} ({num_examples} examples)"
        )
        ...
        logger.info(
            f"Evaluation Complete. Accuracy: {accuracy:.2%} ({correct}/{total})"
        )
